...
class ROE_TTM(SelectionCondition):
    """
    净资产收益率
    ...
    def getData(self):
        ...
        if self.threshold.endswith('TD'):
            ...
            # self.day_head_s = time.strftime('%Y%m%d', time.strptime(self.day_head, '%Y-%m-%d'))
            sql = "select S_INFO_WINDCODE, a.S_DFA_ROE_TTM from (select S_INFO_WINDCODE, S_DFA_ROE_TTM " \
                  "FROM wind.pitfinancialfactor " \
                  "where trade_dt='{}') a left join " \
                  "(select S_INFO_WINDCODE, S_DFA_ROE_TTM FROM wind.pitfinancialfactor " \
                  "where trade_dt='{}') b on a.S_INFO_WINDCODE=b.S_INFO_WINDCODE where a.S_DFA_ROE_TTM '{}'" \
                  "b.S_DFA_ROE_TTM" .format(self.day, self.day_head, self.symbol)
        else:
            # sql = "select S_INFO_WINDCODE, S_DFA_ROE_TTM FROM wind.pitfinancialfactor where trade_dt='{}' and " \
            #       " S_DFA_ROE_TTM {} {}" \
            #     .format(self.day, self.symbol, self.threshold)
            sql = "select S_INFO_WINDCODE, S_DFA_ROE_TTM from wind.pitfinancialfactor " \
                  "where (S_INFO_WINDCODE, TRADE_DT) in (select * from " \
                  "(select S_INFO_WINDCODE, max(TRADE_DT) dt from wind.pitfinancialfactor " \
                  "where trade_dt<='{}' and TRADE_DT>='{}' group by S_INFO_WINDCODE " \
                  "order by S_INFO_WINDCODE,dt desc)) and S_DFA_ROE_TTM {} {}"\
...
if __name__ == '__main__':
    # mode = 'backtest'
    mode = 'live'
    a = ROE_TTM(['-1Y','>=', '16'], '20210426', '20210426', 'dev', mode)
    codes_dict = a.getSecurityPool('ROE_TTM:-1Y,>=,16')
    a.setSecurityPool(codes_dict, 'ROE_TTM:-1Y,>=,16')
    print(1)
