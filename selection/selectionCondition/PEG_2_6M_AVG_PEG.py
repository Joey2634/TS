...
class PEG_2_6M_AVG_PEG(SelectionCondition):
    """
    PEG/过去六个月PEG的均值_PIT[-1TD] <= 1.1
    """
    ...
    def getData(self):
        ...
        # 过去六个月
        self.n_day_head = getTradeSectionDates(self.current_day, (-121))[0]
        if self.threshold.endswith('TD'):
            self.day_head = getTradeSectionDates(self.current_day, int(self.threshold[:-2])-1)[0]
            # 过去六个月
            self.b_day_head = getTradeSectionDates(self.day_head, (-121))[0]
            sql = "select * from " \
    ...
    def getSecurityPool(self, selection_condition):
        ...
        for i in self.trade_dates:
            ...
            print(i, 'PEG_2_6M_AVG_PEG:从WIND数据库获取数据中...')
