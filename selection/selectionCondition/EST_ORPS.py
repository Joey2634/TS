...
class EST_ORPS(SelectionCondition):
    """
    一致预测每股收益(FY1)变化率_1M_PIT[-1TD] >= 0
    """
    ...
    def getData(self):
        ...
        if self.threshold.endswith('TD'):
            ...
            sql = "select a.S_INFO_WINDCODE, a.S_WEST_eps_fy1_1m from " \
                  "(select S_INFO_WINDCODE, S_WEST_eps_fy1_1m FROM wind.consensusexpectationfactor " \
                  "where trade_dt='{}') a left join " \
                  "(select S_INFO_WINDCODE, S_WEST_eps_fy1_1m FROM wind.consensusexpectationfactor " \
                  "where trade_dt='{}') b on a.S_INFO_WINDCODE=b.S_INFO_WINDCODE" \
                  " where a.S_WEST_eps_fy1_1m {} b.S_WEST_eps_fy1_1m"\
                .format(self.day, self.day_head, self.symbol)
        else:
            # sql = "select S_INFO_WINDCODE, S_WEST_eps_fy1_1m FROM wind.consensusexpectationfactor where trade_dt='{}' and " \
            #       " S_WEST_eps_fy1_1m {} {}".format(self.day, self.symbol, self.threshold)
            sql = "select S_INFO_WINDCODE, S_WEST_EPS_FY1_1M from wind.consensusexpectationfactor " \
                  ...
                  "order by S_INFO_WINDCODE,dt desc) group by S_INFO_WINDCODE) and S_WEST_eps_fy1_1m {} {}"\
    ...
    def getSecurityPool(self, selection_condition):
        ...
        for i in self.trade_dates:
            ...
            print(i, 'EST_ORPS:从WIND数据库获取数据中...')
