...
class NETPROFIT_FTM_1M(SelectionCondition):
    """
    ...
    def getData(self):
        ...
        if self.threshold.endswith('TD'):
            ...
            sql = "select a.S_INFO_WINDCODE, a.S_WEST_NETPROFIT_FTM_1M from " \
                  "(select S_INFO_WINDCODE, S_WEST_NETPROFIT_FTM_1M FROM wind.consensusexpectationfactor " \
                  "where trade_dt='{}') a left join " \
                  "(select S_INFO_WINDCODE, S_WEST_NETPROFIT_FTM_1M FROM wind.consensusexpectationfactor " \
                  "where trade_dt='{}') b on a.S_INFO_WINDCODE=b.S_INFO_WINDCODE" \
                  " where a.S_WEST_NETPROFIT_FTM_1M {} b.S_WEST_NETPROFIT_FTM_1M"\
                .format(self.day, self.day_head, self.symbol)
        else:
            # sql = "select S_INFO_WINDCODE, S_WEST_NETPROFIT_FTM_1M FROM wind.ConsensusExpectationFactor where trade_dt='{}' and " \
            #       " S_WEST_NETPROFIT_FTM_1M {} {}".format(self.day, self.symbol, self.threshold)
            sql = "select S_INFO_WINDCODE, S_WEST_NETPROFIT_FTM_1M from wind.ConsensusExpectationFactor " \
                  ...
                  "order by S_INFO_WINDCODE,dt desc) group by S_INFO_WINDCODE) and S_WEST_NETPROFIT_FTM_1M {} {}"\
    ...
    def getSecurityPool(self, selection_condition):
        ...
        for i in self.trade_dates:
            ...
            print(i, 'NETPROFIT_FTM_1M:从WIND数据库获取数据中...')
