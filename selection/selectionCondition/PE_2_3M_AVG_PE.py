...
class PE_2_3M_AVG_PE(SelectionCondition):
    """
    市盈率PE/过去三个月市盈率PE的均值_PIT[-1TD] <= 1.1
    """
    ...
    def getData(self):
        ...
        if self.threshold.endswith('TD'):
            ...
            sql = "select * from " \
                ...
                .format(self.day, self.n_day_head, self.m_day, self.day, self.day_head, self.b_day_head, self.day_head, self.symbol)
        else:
            ...
            #       "group by a.s_info_windcode) c left join (select s_info_windcode,s_val_pe_TTM as PE from ashareeodderivativeindicator " \
            #       "where trade_dt = '{}' and s_val_pe_TTM is not NULL) d on c.s_info_windcode =d.s_info_windcode) where rate {} {}"\
            #     .format(self.m_day, self.n_day_head, self.day, self.symbol, self.threshold)
            sql = "select * from (select c.s_info_windcode,d.PE/c.avge as rate from (select a.s_info_windcode, avg(a.PE) as avge from " \
                  "(select s_info_windcode, trade_dt, s_val_pe_TTM as PE from ashareeodderivativeindicator" \
                  ") a right join (select trade_days as trade_dt from asharecalendar" \
                  " where s_info_exchmarket = 'SSE' and trade_days <= '{}' and trade_days >= '{}') b on a.trade_dt = b.trade_dt " \
    ...
    def getSecurityPool(self, selection_condition):
        ...
        for i in self.trade_dates:
            ...
            print(i, 'PE_2_3M_AVG_PE:从WIND数据库获取数据中...')
...
if __name__ == '__main__':
    mode = 'backtest'
    # mode = 'live'
    a = PE_2_3M_AVG_PE(['-1TD', '<=', '-3TD'], '20210426', '20210426', 'dev', mode)
    codes_dict = a.getSecurityPool('PE_2_3M_AVG_PE:-1TD,<=,1.1')
    a.setSecurityPool(codes_dict, 'PE_2_3M_AVG_PE:-1TD,<=,1.1')
    print(1)
